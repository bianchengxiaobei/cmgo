...
// 目的是解决分布式下生成唯一id 所以ID中是包含集群和节点编号在内的
...
// 实例化一个工作节点
func NewGenerator(serverId int64) (*Generator, error) {
	// 要先检测workerId是否在上面定义的范围内
	if serverId < 0 || serverId > workerMax {
		return nil, errors.New("Generator ID excess of quantity")
	}
	// 生成一个新节点
	return &Generator{
		timestamp: 0,
